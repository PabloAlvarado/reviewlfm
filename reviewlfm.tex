...
\thanks{Simo~S\"arkk\"a is with the Department of Electrical Engineering and Automation (EEA), Aalto University, Rakentajanaukio 2c, 02150 Espoo, Finland (simo.sarkka@aalto.fi). Tel. +358 50 512 4393}% <-this % stops a space
...
\end{equation}
%
where the functions $u(t)$ and $f(t)$ are unknown an $c(t)$ is a control function (known or to be optimized). Further assume that we measure the function $f(t)$ via noisy measurements at discrete instants of time $t_1,t_2,\ldots,t_n$:
%
\begin{equation}
\begin{split}
  y_k &= f(t_k) + \epsilon_k.
...
\end{equation}
%
where the linear operator is typically $\mathcal{L}\equiv a_m \, \frac{d^m}{dt^{m}} + \cdots + a_1 \, \frac{d}{dt} + a_0$ for some fixed constants $a_i, i=0,\ldots,m$. This kind of models can often be rewritten in 
...
\end{equation}
%
where $\mathcal{L}$ is a linear operator in space and time. The input Gaussian process $u(\mathbf{x},t)$ and control input $c(\mathbf{x},t)$ are also space-time processes. Typically, the operator has the form
%
\begin{equation}
  \mathcal{L}\equiv \mathcal{A}_m \, \frac{d^m}{dt^{m}} + \cdots + \mathcal{A}_1 \, \frac{d}{dt} + \mathcal{A}_0,
\label{eq:Lfuc2}
\end{equation}
%
where $\mathcal{A}_0,\ldots,\mathcal{A}_m$ are some spatial partial differential or pseudo-differential operators. This kind of models can often be also written in form of \emph{spatio-temporal state-space models}
%
\begin{equation}
\begin{split}
...
\end{equation}
%
Given the likelihood there are different ways in which the hyperparameters can be estimated, including maximization of the log-marginal likelihood \cite{Rasmussen+Williams:2006}, maximum a posteriori \cite{Murphy:2012:MLP}, and Markov chain Monte Carlo \cite{FilipponeML13} methods. 
...
differential operator of the form $\mathcal{L}\equiv a_m \, \frac{d^m}{dt^{m}} + \cdots
...
%
A convenient way to compute the impulse responses required for the GP inference on LFMs is to rewrite the differential equation in state-space form, which allows us to express the impulse response in terms of matrix exponential function. We can rewrite the basic LFM differential equation \eqref{eq:difeq1} in state-space form \cite{Hartikainen+Sarkka:2011,Hartikainen+Seppanen+Sarkka:2012} by defining a state vector $\mathbf{f} = (f, df/dt,\ldots,d^{m-1}f/dt^{m-1})$ and writing
%
\begin{equation}
\begin{split}
...
\end{align*}
%
The solution for $f(\mathbf{x})$ has the form
\begin{align*}
...
du/dt,\ldots,d^{s-1}u/dt^{s-1})$---note that depending on parameterization the state vector might not contain exactly
...
\end{equation}
%
where $\nabla^2 = \partial^2 / \partial x_1^2 + \cdots + \partial^2 / \partial x_{d-1}^2$ and $\sqrt{\cdot}$ in the matrix $\mathbf{\mathcal{A}}_u$ denotes an operator square root.
...
\begin{equation}
\begin{split}
%  \frac{\partial^2 \tilde{u}(i \boldsymbol{\omega}_x,t)}{\partial t^2}
%  = \| \boldsymbol{\omega}_x \|^2 \tilde{u}(i \boldsymbol{\omega}_x,t) + \tilde{u}(i \boldsymbol{\omega}_x,t)
  \frac{\partial \tilde{\mathbf{f}}(i \boldsymbol{\omega}_\mathbf{x},t)}{\partial t} &= 
       \begin{pmatrix} 
        0 & 1 \\ 
        \| \boldsymbol{\omega}_{\mathbf{x}} \|^2 & 0
      \end{pmatrix} \, \tilde{\mathbf{f}}(i \boldsymbol{\omega}_\mathbf{x},t)
      + \begin{pmatrix} 0 \\ 1 \end{pmatrix} \, \tilde{u}(i \boldsymbol{\omega}_\mathbf{x},t).
\end{split}
\end{equation}
%
which can be seen to correspond to a unstable system, because the transition matrix has the positive eigenvalue $\| \boldsymbol{\omega}_{\mathbf{x}} \|$, which implies a diverging solution.

In order to form a valid state-space model, let us take time-space Fourier transform of \eqref{ipoisson} which gives
%
\begin{equation}
\begin{split}
  (i \omega_t)^2 \, F(i \boldsymbol{\omega}_x,i \omega_t)
  + \| i \boldsymbol{\omega}_x \|^2 F(i \boldsymbol{\omega}_x,i \omega_t)= U(i \boldsymbol{\omega}_x,i \omega_t),
\end{split}
\end{equation}
%
%
implying that the spectral density of $f$ has the form
%
\begin{equation}
\begin{split}
  S_f(\boldsymbol{\omega}_x,\omega_t)
  &= \frac{S_u(\boldsymbol{\omega}_x,\omega_t)}{\left( (\omega_t)^2 \ + \| \boldsymbol{\omega}_x \|^2 \right)^2}
\end{split}
\end{equation}
%
...
\begin{equation}
  \mathcal{C} = \begin{pmatrix}
  	\mathbf{M}_f & \mathbf{A}_f \, \mathbf{M}_f & \hdots & \mathbf{A}_f^{m-1} \, \mathbf{M}_f
  \end{pmatrix}.
\end{equation}
...
is to allow the input force $u(t)$ to be discontinuous. Equally important is to allow the coefficients $\{a_i\}_{i=0}^m$ that
...
with coefficients $\{a_i^j\}_{i=0,j=1}^{m, J}$, and $\{u_j(t)\}_{j=1}^J$ are GPs with covariance functions $k_j(t,t')$.
...
\centering
   ...
   \label{pde:poisson:latent:gt}]{%
     ...
     \subfloat[Contours of the surface in Fig. \ref{pde:poisson:latent:gt} 
...
\label{pde:poisson:latent:pr}]{%
     ...
     \subfloat[Contours of the surface in Fig. \ref{pde:poisson:latent:pr} 
...
observations $y_k$, as in Eq. \eqref{eq:poisson0}. We assume that the source follows
