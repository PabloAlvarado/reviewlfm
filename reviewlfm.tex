...
\simo{{\em Notation:} $n$ is the number of data points, $p$ is the input ($\mathbf{x}$) dimension, $d$ is the vector output dimensionality, $s$ for order of ODE, $k$ is the discrete time index, if we have space and time, $n = n_x \, n_t$ for data points in time and space (for a grid).}
...
The key result is that such a state-space representation exists if and only if the spectral density (the Fourier transform of $k(\tau)$) is a proper rational function:
%
\begin{equation}
\begin{split}
  S(\omega) &= \frac{b_0 + b_1 \, \omega^2 + \cdots + b_L \, \omega^{2L}}{a_0 + a_1 \, \omega^2 + \cdots + a_M \, \omega^{2M}}.
\end{split}
\end{equation}
%
...
For example, the Mat\'ern covariance functions  $k(\tau) = \sigma^2\frac{2^{1-\nu}}{\Gamma(\nu)}\left(\sqrt{2\nu}\,\frac{\tau}{\ell}\right)^\nu K_\nu\left(\sqrt{2\nu}\,\frac{\tau}{\ell}\right)$ with certain parameter values ($\nu=1/2,3/2,\ldots$) have exact state-space representations. In particular, with $\nu=3/2$ we get
 %
\begin{equation}
\begin{split}
