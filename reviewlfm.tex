...
\end{proof}
%
In practical terms it is thus easiest to use, for example, the classical rank-condition (see, e.g., \cite{Ogata:1997}) which says that the (joint) system $(\mathbf{A},\mathbf{C})$ is observable, provided that the following matrix has full rank:
%
\begin{equation}
  \mathcal{O} = 
  \begin{pmatrix}
...
\begin{split}
  ...
  \\
  &\qquad
  + \mathbf{K}_{11} \, \mathbf{M}_f \, \mathbf{U}^{-1} \, \mathbf{M}_f^T \, \mathbf{K}_{11} - \mathbf{X}(t) \\
...
\end{equation}
%
and a solution which only depends on the parameters of the Gaussian latent force model and how the force and control enter the equation (i.e., on $\mathbf{B}_f$ and $\mathbf{M}_f$). This result is summarized in the following theorem which can be derived by setting the time derivatives in Theorem~\ref{the:control1} to zero.
...
\begin{proof}
This can be derived by writing down the output controllability condition and noticing that it reduces to controllability of the physical system part.
\end{proof}
...
%
In the case of PDE LFMs we naturally get models of the form
%
\begin{equation}
\begin{split}
  \frac{\partial \mathbf{g}(\mathbf{x},t)}{\partial t}
  &= \mathbf{\mathcal{A}} \, \mathbf{g}(\mathbf{x},t)
  + \mathbf{B} \, w(\mathbf{x},t) + \mathbf{\mathcal{M}} \, c(\mathbf{x},t) \\
  y_k &= \mathbf{C} \, \mathbf{g}(t_k) + \epsilon_k.
\end{split}
\end{equation}
%
...
As shown in \cite{Sarkka:2011,Sarkka+Hartikainen:2012,Sarkka+Solin+Hartikainen:2013} Gaussian process regression models easily extend to having linear operators or functionals $\mathcal{H}_k$ in the measurement model:
%
\begin{equation}
...
necessary to resort to approximations in order to compute the posterior distribution for $u^*$ given $\mathbf{y}$, or the predictive distribution for $f^*$ given $\mathbf{y}$. Approximation methods for the covariance function formulation of LFMs have been presented in \cite{Lawrence:gpsim2007a,Gao:latent08,Titsias:BMC:2012}, In the state-representation, the non-linearity transform the linear state-space model into a non-linear state-space model, where we can use non-linear Kalman filtering and smoothing methods \cite{Hartikainen+Seppanen+Sarkka:2012,Sarkka:2013}. The stochastic control problems could also be extended to the non-linear case.
