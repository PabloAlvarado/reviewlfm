...
\simo{Mauricio: check the notation on inputs $\mathbf{x}$ vs. $\mathbf{x},t$ vs. $t$}
...
Fig.~\ref{reg_ex} shows an example of Gaussian process regression. \simo{Mauricio: Explain more.}

\simo{Mauricio: explain how we can also estimate the parameters using ML, MAP, or some fancier method such as MCMC, slice-samplings, CCD.}
...
%
\simo{Mauricio: check the notation}
...
conditions. For simplicity in the presentation, we assume that the boundary conditions are zero, but the boundary conditions could also be incorporated. In this case the Green's function $H(\mathbf{x}, \mathbf{z})$, with suitable boundary conditions, can be written in closed form.  \simo{Mauricio: Find a citation for this closed form expression}
...
\end{align*}
%
and again the Green's function $H$ it can be written in closed form. \simo{Mauricio: Find a reference for this}
...
\simo{Mauricio: just explain that we can use the ordinary GP hyperparameter learning once we have the covariance functions.}
...
\simo{Mauricio: double check your text.}
...
\end{align*}
where $s^2 = 1$.
Figure XX shows the input source in the domain $[-2, 2]\times [-2,2]$.

\begin{figure*}[!ht]
\centering
   \subfloat[
    %ODE output $f(t)$ and measurements $\{y_k\}_{k=1}^n$. The dashed line is the ground truth output $f(t)$
   %obtained by solving the ODE, and the dots
   %represent the noisy measurements $\{y_k\}_{k=1}^n$. The solid line is the predictive mean $f(t_*)$, and the grey
   %region represents the predictive uncertainty, two standard deviations away from the mean prediction.
\label{pde:poisson:latent:gt}]{%
       \includegraphics[width=0.33\textwidth]{toyPDElatentGT}
         }
     \subfloat[
     %Ground truth and predicted source $u(t)$. The dashed line is the ground truth source $u(t)$. The solid
     %line is the mean posterior for  $u^*$, while the shaded region covers two standard deviation away from the mean. 
...
The MSE between the ground truth source $u(x,y)$, and the inferred mean source $u^*(x,y)$ is equal to $0.5153$. The MSE
between the ground truth output $f(x,y)$ and the mean predicted output $f^*(x,y)$ evaluated at the test input points is
$8.3354\times 10^{-4}$.
