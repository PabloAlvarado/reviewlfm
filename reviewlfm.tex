...
\author{Mauricio~A.~\'Alvarez, \simo{$\rightleftarrows$}
         Simo~S\"arkk\"a,
         \simo{and~Neil~D.~Lawrence (?)}% <-this % stops a space
...
State-space realizations of spatio-temporal oscillating fields have been discussed in \cite{Solin+Sarkka:2013physrev}. The implementation of divergence-free covariance functions for magnetic field modeling have been recently presented in \cite{Solin_et_al_magnetic}.
...
\simo{Extend the same idea PDEs, but with operator exponentials}
