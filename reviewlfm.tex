...
\ref{Alvarez+Luengo+Lawrence:2013}.
 
As an example, we assume that the input source $u(t)$ is given as
\begin{align*}
u(t) = \sin(0.37t +1.22) + \sin(0.29t + 0.38),
\end{align*}
where $t\in [0,50]$. Figure XX shows the input $u(t)$ computed for
$N=5000$ points equally distributed in the input domain. 

Parameters for the ODE are $\lambda = 0.1$, and $\gamma = 1$. The
output function $f(t)$ obtained by solving the differential equation
is shown in Figure XX. We randomly choose $n=500$ points for $f(t)$
for training, evaluated at $\{t_k\}_{k=1}^n$, and add Gaussian noise
$\epsilon_k$ with zero mean and variance equal to $1e-4$, to get
