function g = sheatKernGradientErfc(sigmax, lengthX, s1, s2, w, gamma, ...
    pwz1, pwz2, n, m, covGrad, bctype, isNumeric)
...
%     g = dC*sum(sum((cosS1*cosS2').*covGrad));
    ...
    if isNumeric
        ...
        dC = integral2(z, 0, lengthX, 0, lengthX, tol);
        dC = (2/sigmax^3)*dC;
    else
        if n == m          
            Wox = exp((gamma(n)*sigmax/2)^2)*(Faddeeva_erfc(pwz1(n)) - Faddeeva_erfc(pwz2(n)));
            
            dWox = ((gamma(n)^2)*sigmax/2)*Wox - ...
                (2/sqrt(pi))*exp((gamma(n)*sigmax/2)^2)*(exp(-(pwz1(n)^2))*(gamma(n)/2) - ...
                exp(-(pwz2(n)^2))*(-lengthX/(sigmax^2) + gamma(n)/2));            
            ...
            if mod(n+m,2)==1
                ...
                dWoxm = ((gamma(m)^2)*sigmax/2)*Woxm - ...
                (2/sqrt(pi))*exp((gamma(m)*sigmax/2)^2)*(exp(-(pwz1(m)^2))*(gamma(m)/2) - ...
                exp(-(pwz2(m)^2))*(-lengthX/(sigmax^2) + gamma(m)/2));

                dWoxn = ((gamma(n)^2)*sigmax/2)*Woxn - ...
                (2/sqrt(pi))*exp((gamma(n)*sigmax/2)^2)*(exp(-(pwz1(n)^2))*(gamma(n)/2) - ...
                exp(-(pwz2(n)^2))*(-lengthX/(sigmax^2) + gamma(n)/2)); 
