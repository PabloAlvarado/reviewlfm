%
% Demonstrate GP regression with SE covariance
%
...
%%
% Plot the above example properly
%
    clf;
...
%
    ...
    dim = dim_list_all{n}(order)
    
    ks_mu = ks_mu_all{n,order};
    ks_V  = ks_V_all{n,order};
    ...
    hold on;
    alpha(0.5);
    h = plot(TT,XX,'k--',T,Y,'ro',TT,ks_mu,'k-');
    set(h,'Linewidth',2);
    set(h,'MarkerSize',10);
    grid on;
    axis([0 11 -1.5 1.5]);

    plot(TT,ks_mu,TT,ks_mu-1.96*sqrt(ks_V),'--',TT,ks_mu+1.96*sqrt(ks_V),'--');
...
%    
    ...
    clf;
    for n=1:length(dim_list_all)
        dim_list = dim_list_all{n};
        mean_err = mean_err_all{n};
        var_err  = var_err_all{n};

        ind = find((dim_list <= max(order_list)) & (dim_list >= 2));
        
        semilogy(dim_list(ind),mean_err(ind),'*-',dim_list(ind),var_err(ind),'v--');
%        plot(dim_list(ind),mean_err(ind),'*-',dim_list(ind),var_err(ind),'v--');
        grid on;
        hold on;
    end
...
%%
% Regression example with Taylor
%
    clf;
    set(gcf,'PaperType','a4');
    set(gcf,'PaperUnits','inches');
...
%        subplot(2,2,i);
        if i > 1
            order = ords(i-1);
            m1 = ks_mu_all{n,order};
            a1 = ks_mu_all{n,order} + 1.96*sqrt(ks_V_all{n,order});
            b1 = ks_mu_all{n,order} - 1.96*sqrt(ks_V_all{n,order});    
            tit = sprintf('Taylor %d',order);
        else
            m1 = mu;
            a1 = mu + 1.96*sqrt(V);
            b1 = mu - 1.96*sqrt(V);    
        ...
        clf;
        p=patch([TT fliplr(TT)], [a1 fliplr(b1)], 1);
        set(p,'EdgeColor','none')
        hold on;
        alpha(0.5);
        h = plot(TT,XX,'k--',T,Y,'ro',TT,m1,'k-');
        set(h,'Linewidth',2);
        set(h,'MarkerSize',10);
        grid on;
        axis([0 11 -1.5 1.5]);
        ...
        print('-depsc',sprintf('taylor_reg%d',i));
        %title(tit);
    end

%%
% Comparison of Taylor 4 and Pade 2/4
%
    m0 = mu;
    a0 = mu + 1.96*sqrt(V);
    b0 = mu - 1.96*sqrt(V);    
    ...
    dim_list_all{n}(order)

    n = 5;
    order = 1;
    m2 = ks_mu_all{n,order};
    a2 = ks_mu_all{n,order} + 1.96*sqrt(ks_V_all{n,order});
    b2 = ks_mu_all{n,order} - 1.96*sqrt(ks_V_all{n,order});    
