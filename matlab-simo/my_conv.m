 function w = my_conv(u,v)
   % MY_CONV Nonnumeric convolution, i.e., works also with symbols.
   %
   % W = MY_CONV(U,V) is the convolution of symbolic (or numerical)
   % vectors U and V. length(w) = length(u)+length(v)-1
